...
export async function GET() {
  ...
  const propertiesCount = (await db.select({ count: sql<number>`count(*)` }).from(properties).get())?.count ?? 0;
  const availableCount = (await db.select({ count: sql<number>`count(*)` }).from(properties).where(eq(properties.status, "available")).get())?.count ?? 0;
  const tenantsCount = (await db.select({ count: sql<number>`count(*)` }).from(tenants).where(eq(tenants.leaseStatus, "active")).get())?.count ?? 0;
  const newPrescreenings = (await db.select({ count: sql<number>`count(*)` }).from(prescreenings).where(eq(prescreenings.status, "new")).get())?.count ?? 0;
  ...
  const openMaintenance = (await db.select({ count: sql<number>`count(*)` }).from(maintenanceRequests)
